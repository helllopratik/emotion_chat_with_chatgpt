...
def on_send():
    ...
    if user_input:
        ...
        def detect_and_respond():
            ...
            response_prompt = (
                f"Imagine you are a human-like conversational assistant. The user is feeling {detected_emotion}. "
                f"They asked: '{user_input}'. "
                "Respond in a friendly, empathetic manner while considering their emotional state."
            )

            response = model.start_chat(history=[]).send_message(response_prompt)
